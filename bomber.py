...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/D3V_S4NJ4Y/SMSBOMB/archive/dev.zip"
        dir_name = "SMSBOMB-dev"
    else:
        zip_url = "https://github.com/D3V_S4NJ4Y/SMSBOMB/archive/master.zip"
    ...
    if success:
        mesgdcrt.SuccessMessage("SMSBOMB was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update SMSBOMB.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/D3V_S4NJ4Y/SMSBOMB.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/D3V_S4NJ4Y/SMSBOMB.git HEAD")
    sys.exit()
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("SMSBOMB was created by D3V_S4NJ4Y")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("SMSBOMB will work only in Python v3")
    sys.exit()
...
description = """SMSBOMB - Your Friendly Spammer Application

SMSBOMB can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

SMSBOMB is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by D3V_S4NJ4Y !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start SMSBOMB with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start SMSBOMB with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start SMSBOMB with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update SMSBOMB")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current SMSBOMB contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current SMSBOMB version")
